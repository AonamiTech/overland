  ...
  try {
    ...
    if (eventType === 'bid_placed') {
      ...
      text = `A new bid of $${rec.amount ?? 0} was placed on your listing (${listing.origin} → ${listing.dest}).\n\n` +
        `Sign in to Overland to review bids and connect: https://overland-ochre.vercel.app/board\n\n` +
      ...
      text = `Your bid of $${rec.amount ?? 0} for ${listing.origin} → ${listing.dest} was accepted!\n\n` +
        `You can now connect directly with the load poster on Overland: https://overland-ochre.vercel.app/board\n\n` +
